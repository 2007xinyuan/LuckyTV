...
import com.lucky.kidstv.bean.SubtitleBean;
import com.lucky.kidstv.bean.VodInfo;
import com.lucky.kidstv.cache.CacheManager;
import com.lucky.kidstv.event.RefreshEvent;
import com.lucky.kidstv.player.EXOmPlayer;
import com.lucky.kidstv.player.IjkmPlayer;
import com.lucky.kidstv.player.MyVideoView;
import xyz.doikki.videoplayer.player.AndroidMediaPlayer;
import com.lucky.kidstv.player.TrackInfo;
import com.lucky.kidstv.player.TrackInfoBean;
import com.lucky.kidstv.player.controller.VodController;
import com.lucky.kidstv.player.danmu.Parser;
import com.lucky.kidstv.player.thirdparty.Kodi;
import com.lucky.kidstv.player.thirdparty.MXPlayer;
import com.lucky.kidstv.player.thirdparty.ReexPlayer;
...
import com.lucky.kidstv.ui.dialog.SearchSubtitleDialog;
...
import com.lucky.kidstv.util.M3U8;
import com.lucky.kidstv.util.MD5;
import com.lucky.kidstv.util.PlayerHelper;
import com.lucky.kidstv.util.parser.SuperParse;
import com.lucky.kidstv.util.StringUtils;
import com.lucky.kidstv.util.SubtitleHelper;
import com.lucky.kidstv.util.VideoParseRuler;
import com.lucky.kidstv.util.XWalkUtils;
import com.lucky.kidstv.util.thunder.Jianpian;
import com.lucky.kidstv.util.thunder.Thunder;
...
//                }
    ...
    private void toggleAdMark() {
        ...
        if (adMarkStartMs < 0) {
            adMarkStartMs = pos;
            Toast.makeText(this, "已标记广告起点(" + (pos / 1000) + "s)，播到广告结束再按一次", Toast.LENGTH_LONG).show();
        } else {
            long start = adMarkStartMs;
            long end = pos;
            adMarkStartMs = -1;
            if (end <= start + 1000) {
                Toast.makeText(this, "广告段太短，未保存", Toast.LENGTH_SHORT).show();
                return;
            }
            saveAdSegment(videoURL, start, end);
            Toast.makeText(this, "广告段已保存(从" + (start / 1000) + "s到" + (end / 1000) + "s)，下次播放自动跳过", Toast.LENGTH_LONG).show();
        }
    }
